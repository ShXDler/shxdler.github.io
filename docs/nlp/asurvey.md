...
![img](https://pic3.zhimg.com/v2-440d731b31a485f3fe0e3e0472412822_b.png)
...
![img](https://pic1.zhimg.com/v2-4ce7f41e693f77923eb5037ecff5bd90_b.png)
...
文本生成的过程可以抽象地看成序列型多标签分类问题，可以直接通过负对数似然函数（NLL）进行优化，通过极大似然估计得到的目标函数如下： ${\cal L}_{NLL}(\theta)=-\log_{p_\theta}(Y|X)=-\sum^m_{t=1}\log(p_\theta(y_t|y_{<t},X))$ 
...
![img](https://pic3.zhimg.com/v2-29e85a37421792943bd7ce1d503acfde_b.png)
...
![img](https://pic3.zhimg.com/v2-43bc309ef3e7e831b787b1e068934eb2_b.png)
...
CopyNet有着独特的网络架构，可以通过端到端的方式进行训练。在CopyNet中，生成目标单词的概率是两种模式概率的组合，**生成模式**和**拷贝模式**。首先，CopyNet对全局词汇表 $\cal V$ 和源序列词汇表 ${\cal V}_x$ 中每一个单词进行表示，构建了扩增词汇表 ${\cal V}_{ext}={\cal V}\cup{\cal V}_x\cup\{\rm unk\}$ ，得到扩增词汇表 ${\cal V}_{ext}$ 上的概率分布 $p(y_t)=p_g(y_t)+p_c(y_t)$ ，等式右边分别代表下式计算得到的**生成模式**和**拷贝模式**的概率：

![img](https://pic4.zhimg.com/v2-7cd5057acef05cb97a16da8d49381edb_b.png)

其中 $Z$ 为两种模式共享的标准化项， $\psi_g(\cdot)$ 和 $\psi_c(\cdot)$ 分别代表两种模式的得分函数：

![img](https://pic2.zhimg.com/v2-1524cc7d64e50a0affb0450de6fcc1f1_b.png)

其中 ${\bf v}_i$ 为 $v_i$ 的one-hot示性向量， $\psi({\cdot})$ 也存在其他的形式。

**指针生成器网络（Pointer-Generator Network, PGN）**

PGN也有独特的网络架构。不同的是，PGN补充了生成模式和拷贝模式之间的**切换**概率 $p_m$ ，得到的 ${\cal V}_{ext}$ 上的概率分布 $p(y_t)=p_m({\rm g})\cdot p_g(y_t)+(1-p_m({\rm g}))\cdot p_c(y_t)$ 。这里切换概率 $p_m$ 代表选择生成模式的概率：

![img](https://pic1.zhimg.com/v2-2e4c483e5cbad24612b9d859f883a6e0_b.png)

而上式中注意力系数 $\alpha_{tj}$ 与前文计算方法相同，词汇表概率分布则计算如下：

![img](https://pic1.zhimg.com/v2-6861a4105ee8e551ab0f23a7949aca78_b.png)
...
![img](https://pic3.zhimg.com/v2-697e303e42e084b695ece3e9e5affac6_b.png)
...
![img](https://pic3.zhimg.com/v2-a8cea5e07f77fa40483aa65b0e63fc12_b.png)

这里 ${\cal N}(u)=\{(u_i,e_{ij},u_j)\in{\cal E}|u_i=u{\rm \ or\ }u_j=u\}$ 代表包含节点 $u$ 的边的集合， ${\bf u}^{(k)}$ 和 ${\bf e}_{ij}^{(k)}$ 代表对应节点和对应边在第 $k$ 次迭代的特征向量。 $\rm A{\scriptstyle GGREGATE}(\cdot)$ 函数通常被用在标注图中作为关系图建模的GNN，比如在GAT中：

![img](https://pic2.zhimg.com/v2-4cd5f71e554bd250e7d6ddc2d4c8fe01_b.png)

其中 $\sigma(\cdot)$ 是非线性的激活函数（通常使用LeakyReLU），而 ${\bf W}_i$ ，${\bf W}_j$，${\bf W}_k $ 则均为可训练的权重矩阵。图注意力机制也可以是多头注意力的，其中的注意力权重衡量了连接两个节点的边的联系程度。另外，研究人员经常使用 ${\bf h}_{\cal G}=\rm {R}\scriptstyle{EADOUT}(\{\bf {u}^{(K)},u\in{\cal U}\})$ 来得到图 $\cal G$ 的表示， $\rm {R}\scriptstyle{EADOUT}(\cdot)$ 常用平均或池化函数等简单的排列恒等函数。
...
后验正则化（posterior regularization, PR）框架被用来约束模型未标注数据后验空间，广泛用于概率模型上添加知识约束条件。PR在训练目标函数 ${\cal L}(\theta)$ 上添加了一个约束项来对相关知识进行编码，记约束函数为 $g(X,Y)\in{\mathbb R}$ ，函数值越大代表特定知识下生成的 $Y$ 越好。为了添加约束条件，PR引入了辅助分布 $q(Y|X)$ ，并在 $q$ 上通过添加促进 $g$ 的期望值更大的约束： $\mathbb{E}q[g(X,Y)]$ 。同时也使用了KL散度促进模型 $p_\theta$ 和 $q$ 接近。由此，该优化问题约束条件可以表示如下：
$$
\max_{\theta,q}{\cal L}(\theta)-{\rm KL}(q(Y|X)||p_{\theta}(Y|X))+\xi\\s.t.{\mathbb E}q[g(X,Y)]>\xi\\
$$
...
![img](https://pic2.zhimg.com/v2-540a4422e57fc34bd762d3a67f15c5ed_b.png)
...
![img](https://pic4.zhimg.com/v2-47ed7a0abf590e577c4290991024f13b_b.png)
...
![img](https://pic4.zhimg.com/v2-62d8363cde382e4dac1b6616db2d2ba7_b.png)
...
![img](https://pic1.zhimg.com/v2-751c1391635cf1c8adb864ba95c3c94c_b.png)
...
![img](https://pic2.zhimg.com/v2-a446476884abc77d8d2cbb22ca561d39_b.png)
...
![img](https://pic4.zhimg.com/v2-4b638b1941ffe18ef0c40cbcc667f55f_b.png)
...
![img](https://pic4.zhimg.com/v2-7e83abbd188d2a191b50fef48d32f84b_b.png)

为了最大化对数似然函数 $\log p(Y,d|X)$ ，作者构建了变分目标函数：

![img](https://pic4.zhimg.com/v2-607965c3ae67d5530826aeef628b7987_b.png)

其中 $q(z|X)$ 是 $z$ 的变分分布。最后再组合两个目标函数 ${\cal J}={\cal J}_{topic}+{\cal J}_{seq2seq}$ 。
...
分配关键词最直接的方法就是从预定义词库中选择关键词。然而，有的输入序列并没有明确的关键词，但我们可以从词库中找到一个。Li等人更详细地描述了这一问题，他们将编码器得到的隐状态的和输入进一个NLP中，以得到情感分类，并以此指导文本生成。 具体来讲，预测情感分类 $k\in{\cal K}$ 首先由一个实值低维向量 $\bf k$ 进行表示，再和上下文向量 ${\bf c}_t$ 进行结合，得到新的向量来生成文本。为了对情感的表达多少进行动态跟踪，Zhou等人在解码过程中使用了一个记忆模块 ：在解码过程开始之前，每个分类先用一个情感状态向量进行初始化，在每一步中，情感状态会以一定速率衰减，解码完成后将会衰减到0，代表情感已经表达完全。Xu等人则通过学习结合情感的回复生成和查询生成的双重任务，对二者的相互关系进行了建模。 
...
![img](https://pic4.zhimg.com/v2-0702b86b58c375e3a3e75d2d114a5f97_b.png)
...
![img](https://pic1.zhimg.com/v2-632e95c5a467778d9d7516bdf6dfd280_b.png)
...
![img](https://pic1.zhimg.com/v2-14535027951ddcd1e1302500f174ef90_b.png)
...
${\cal L}_{CM}(\theta)={\cal L}_{content}(\theta)+{\cal L}_{style}(\theta)=\log p_\theta(X'|k',X)+\log p_\theta(X|k,X)\\$ 
...
![img](https://pic1.zhimg.com/v2-68b4f326fb4bff3283485050fb7fb96c_b.png)
...
![img](https://pic3.zhimg.com/v2-f56be996396f4a5bfa2732fca2d91072_b.png)

### 结合**知识图谱嵌入（KGE）**的语言生成

KGE技术能从KG中学习并将实体和关系嵌入进连续向量空间中，但是KGE只能依赖单跳关系路径，受到得分函数的严格限制。因为KGE能够提供实体节点间的连接性信息，所以能够捕捉到实体节点间的语义联系。这一主要思想是将实体和联系表示在低维空间 ${\mathbb R} ^d, d\ll|{\cal U}\cup{\cal R}|$ 中，从而在保留KG固有结构的同时对数据进行降维。这其中，TransE由其简易和有效的特点成为最常用的技术之一。在给定KG边 $(u_i,r,u_j)$ 后，TransE会将关系看做转义向量 $\bf r$ ，从而嵌入实体 ${\bf u}_i$ 和 ${\bf u}_j$ 可以以较低错误进行连接： ${\bf u}_i+{\bf r}\approx{\bf u}_j$ ，而构建负例的常用方法是替换KG边中的头或尾实体。接着，低维实体和关系的嵌入会通过SGD和L2正则化方法进行优化：

![img](https://pic2.zhimg.com/v2-c9514d75e750eb6f91d0d51ed306d6ed_b.png)
...
**解码增强**：在解码过程中，解码器会在隐藏层表示和序列相关子图 ${\cal G}_{\rm sub}$ 中的边集 ${\cal E}_{\rm sub}$ 中使用注意力机制来更新状态 ${\bf s}_t$ ： ${\bf s}_t=f_{de}({\bf s}_{t-1},{\bf c}_t,{\bf c}_{t-1}^{ke},{\bf e}(y_{t-1}))$ ，其中 ${\bf c}_t$ 是注意力机制中的隐藏层加权和， ${\bf c}_t^{ke}$ 计算每一条边的注意力。
...
基于强化学习的路径搜寻方法通常包括两部分：首先获得从 $\cal G$ 中获得起始节点 $u_0$ ，进行多跳图谱推理，到达包含相应知识的目标节点 $u_k$ ；第二步是使用两个分离的编码器对输入序列 $X$ 和路径 $\Phi_k(u_0,u_k)$ 进行表示，并通过注意力机制进行解码。基于路径的知识图谱推断将图结构转化成了路径的线性结构，使得它能够被序列编码器表示。强化学习的总体生成损失函数定义为 ${\cal L}_{KG-RL-NLL}(\theta)=-\sum^m_{t=1}\log(p(y_t|y_{<t},\Phi_k(u_0,u_k),X))$ 。
...
在编码阶段，结合KG的一个普遍方法是通过结合相应实体节点向量的方法增强单词的语义，使用一个事先定义好的实体连接函数 $\psi:{\cal V\times X\rightarrow U}$ 将输入序列中的单词表示成KG中相应的实体节点。对一个输入序列而言，所有连接的实体和他们的K跳近邻都会被表示成序列相关的K跳子图 ${\cal G}_{sub}$ ，该子图同样使用KG的结构及其实体和边的特征来学习每个实体节点的表示向量 $\bf u$ ，再通过 ${\rm R\scriptstyle EADOUT}(\cdot)$ 函数学习子图表示 ${\bf h}_{subG}={\rm R\scriptstyle EADOUT}(\{\bf {u},u\in{\cal U}_{sub}\})$ 。
...
![img](https://pic2.zhimg.com/v2-a5f40db11d2937551858fa0490cc3619_b.png)
...
![img](https://pic3.zhimg.com/v2-eff8200190bfb85369da37e223568d02_b.png)
...
$\begin{align} {\cal L}_{ST}(\theta)&={\cal L}_{Rerank}(\theta)+{\cal L}_{ST-NLL}(\theta)\\&=-[S^*(T,Y)\log S(T,X)+\log(p(y_t|y_{<t},X,{\rm argmax}_{T\in{\cal T}}p(T|X)))] \end{align}\\$ 
...
![img](https://pic2.zhimg.com/v2-9720d43e1df79c06cbe7924905f5d941_b.png)
